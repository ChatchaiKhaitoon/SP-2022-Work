...
function calculate() {
    ...
    let moneyLevel;
    if (savingRatio <= 9 && debtRatio >= 40 && emergencyFundRatio <= 0 && netWorth <= 0) {
      moneyLevel = "LV.1";
    } else if (savingRatio <= 10 && debtRatio <= 50 && emergencyFundRatio <= 0 && netWorth <= 0) {
      moneyLevel = "LV.2";
    } else if (savingRatio >= 10  && debtRatio <= 40 && emergencyFundRatio >= 1 && emergencyFundRatio <= 6 && netWorth > 0) {
      moneyLevel = "LV.3"; 
    } else if (savingRatio >= 10  && debtRatio <= 40 && emergencyFundRatio >= 6 && emergencyFundRatio <= 12 && netWorth > 0) {
      moneyLevel = "LV.4";
    } else if (savingRatio >= 10  && debtRatio <= 40 && emergencyFundRatio >= 12 && netWorth > 0) {
      moneyLevel = "LV.5";
    } else if (savingRatio >= 10  && debtRatio < 40 && emergencyFundRatio >= 12 && netWorth > 0) {
      moneyLevel = "LV.6";
    } else if (savingRatio >= 15 && debtRatio < 40 && emergencyFundRatio > 12 && netWorth > 0) {
      moneyLevel = "LV.7";
    } else if (savingRatio >= 20 && debtRatio < 40 && emergencyFundRatio > 12 && netWorth > 0) {
      moneyLevel = "LV.8";
    } else{
      moneyLevel = "Lv.1";
    } 
    ...
    resultElement.innerHTML = `
      <div class="card">
        <h2>Your Money Index Result</h2>
        <p>Saving Ratio: ${savingRatio}%</p>
        <p>Debt Ratio: ${debtRatio}</p>
        <p>Emergency Fund Ratio: ${emergencyFundRatio}</p>
        <p>Net Worth: ${netWorth}</p>
        <h3>You are at ${moneyLevel}</h3>
      </div>
    `;
  }
